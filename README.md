...
- A standard library of controls for users to start off with. The repository for this is [over here](https://github.com/jessstrap/servotk_controls) (currently empty). Using these controls should be declarative. Probably by namespaced values for the class attribute and a single onLoad function that iterates over dom values and sets properties.
